	...
	$scope.getPaths = function(){
		...
		$http.post(
	        ...
	        if(status == "200" && data.status == "OK"){
	        		...
	        		console.log('nglViewerLigandController loadNgl WARNING: not prepared yet!');
	        } else {
	        		console.log('nglViewerLigandController loadNgl ERROR: response error!');
	        }
		}).error(function(data, status, headers, config){
			console.log('nglViewerLigandController loadNgl ERROR: critical error!');
		});
