var app = angular.module("nglViewerProteinApp", []); 

app.controller("nglViewerProteinController", [ '$scope','$http', function($scope, $http) {
	
	console.log("Loading nglViewerProteinController...");
	...
	$scope.getPaths = function(){
		...
		console.log("nglViewerProteinController getPaths... ");
		
		$http.post(
				'../action/nglViewerAction.php',
				{					
					'action': 'getProteinFilePaths',					
				}				
		).success(function(data, status, headers, config){
			
	        if(status == "200" && data.status == "OK"){
	        		console.log('nglViewerProteinController loadNgl success!');
	        		$scope.outputPath = data.data;
	        		$scope.loadNgl($scope.outputPath);	        		
	        }
		}).error(function(data, status, headers, config){
			console.log('nglViewerProteinController loadNgl fail!');	
		});	
	...
	$scope.loadNgl = function(path){
		...
		//console.log("nglViewerProteinController loadNgl " +path);
