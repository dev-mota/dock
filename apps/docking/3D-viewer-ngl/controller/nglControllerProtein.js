...
app.controller("nglControllerProtein", [ '$scope','$http', function($scope, $http) {
	
	// console.log("nglControllerProtein...");
	
	$scope.init = function(){
		// console.log("nglControllerProtein init...");
		$scope.getPaths();
	}
	
	$scope.getPaths = function(){
		
		console.log("nglControllerProtein getPaths... ");
		...
		$http.post(
	        ...
	        if(status == "200" && data.status == "OK"){
	        		console.log('nglControllerProtein getSingleFilePath OK: success!');
	        		
	        		// $scope.outputPath = data.data;	        		
	        		var resultArrray = data.data;
	        		$scope.outputPath = resultArrray[0].paths[0].path;
	        		
	        		$scope.loadNgl($scope.outputPath);	        		
	        } else if (data.status == "NOT_PREPARED_YET"){
	        		// console.log('nglControllerProtein loadNgl WARNING: not prepared yet!');
	        } else {
	        		console.log('nglControllerProtein getSingleFilePath ERROR: response error!');
	        }
		}).error(function(data, status, headers, config){
			console.log('nglControllerProtein getSingleFilePath ERROR: critical error!');
		});	
		
	}
	
	$scope.loadNgl = function(path){
		//console.log("nglControllerProtein loadNgl " +path);		
		loadStructure(path).then(function () {})
	}
	...
	function addElement (el) {
	  ...
	  $scope.stage.viewer.container.appendChild(el)
	}
	...
	function loadStructure (input) {
	  ...
	  return $scope.stage.loadFile(input).then(function (o) {
	    struc = o
	    setLigandOptions()
	    setChainOptions()
	    setResidueOptions()
...
//	    }
	...
	}, { top: getTopPosition(60), left: "12px",  width: "130px", "text-align": "left", padding:"1px 6px" }) // getTopPosition(0) = altura acima do botao
...
//	    sidechainAttached ? "(" + neighborSele + ") and (sidechainAttached or not polymer)" : neighborSele
	...
	}, { top: getTopPosition(30), left: "12px", width: "130px", padding:"3px" })
	...
	//  var sceneRadius = $scope.stage.viewer.boundingBox.getSize().length() / 2
	//
